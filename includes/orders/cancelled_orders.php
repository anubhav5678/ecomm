...
$query = query("SELECT * FROM orders INNER JOIN products ON orders.product_code = products.product_code WHERE user_code = '{$_SESSION['user_code']}' AND order_user_status = 'cancelled' ORDER BY orders.order_id DESC ");
confirmQuery($query);
?>
<?php
...
<?php else: ?>
<?php include("includes/orders/warning.php"); warning("You haven't cancelled any order yet!"); ?>
<?php endif; ?>